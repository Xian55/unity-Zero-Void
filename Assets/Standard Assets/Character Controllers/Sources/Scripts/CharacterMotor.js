...
class CharacterMotorMovement {
	...
	var maxForwardSprintSpeed : float = 20.0;
...
function Enable(flag : boolean) { enabled = flag; }
...
function MaxSpeedInDirection (desiredMovementDirection : Vector3) : float {
	if (desiredMovementDirection == Vector3.zero)
		...
		////Little Buggy
		//if(desiredMovementDirection.z > 0 && IsSprinting())
		//	zAxisEllipseMultiplier = movement.maxForwardSprintSpeed;
		
		var temp : Vector3 = new Vector3(desiredMovementDirection.x, 0, desiredMovementDirection.z / zAxisEllipseMultiplier).normalized;
		var length : float = new Vector3(temp.x, 0, temp.z * zAxisEllipseMultiplier).magnitude * (IsSprinting() ? movement.maxForwardSprintSpeed : movement.maxSidewaysSpeed);
		return length;
	}
}
